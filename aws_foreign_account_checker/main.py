...
account_numbers_allowed = set(('736815776541', '831269439992', '349546234265', '944132310209', '972874111695',
...
def get_and_check_policy(retries: int, is_named: bool, role_name='', policy_name='', policy_arn=''):
    ...
    try:
        if is_named:
            policy_paginator = iam_client.get_paginator('list_policy_versions') # type: mypy_boto3_iam.ListPolicyVersionsPaginator
            policy_versions = policy_paginator.paginate(PolicyArn=policy_arn)
            version: str
            ...
            policy_version = iam_client.get_policy_version(PolicyArn=policy_arn, VersionId=version)
            policy_document = json.dumps(policy_version['Document'])
        else:
            policy = iam_client.get_role_policy(RoleName=role_name, PolicyName=policy_name)
            policy_document = json.dumps(policy['PolicyDocument'])
        print(f'policy details: {policy_document}')

        policy_account_numbers = set(account_pattern.findall(policy_document))
        if policy_account_numbers:
            print(policy_account_numbers)
